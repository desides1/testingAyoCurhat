        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    <a target="_blank" href="{{ route('report.download', ['year' => $year, 'month' => request('month')]) }}"
                        class="btn btn-primary mr-2 col-2">Unduh PDF</a>
                </div>
                            ...
                            <select class="form-control" id="select-reporting-year-filter" name="year">
                                ...
                                @foreach ($validYear as $availableYear)
                                    <option value="{{ $availableYear }}"
                                        {{ request('year') == $availableYear ? 'selected' : '' }}>
                                        {{ $availableYear }}
                                    </option>
                                @endforeach
                            </select>
